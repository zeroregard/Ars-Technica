...
public class GenericModel<T extends GeoAnimatable> extends GeoModel<T> {
    ...
    public GenericModel(String modId, String name) {
        this.modelLocation = ResourceLocation.fromNamespaceAndPath(modId, "geo/" + name + ".geo.json");
        this.textLoc = ResourceLocation.fromNamespaceAndPath(modId, "textures/" + textPathRoot + "/" + name + ".png");
        this.animationLoc = ResourceLocation.fromNamespaceAndPath(modId, "animations/" + name + "_animations.json");
        this.name = name;
    }
